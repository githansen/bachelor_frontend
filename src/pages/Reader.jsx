...
function TextPanel({ state, statess }) {
	...
	const textEdit = [
		...
		{
		  name: 'Tilbakestill',
		  action: () => {setFontsize(28); setFontfamily('Arial')},
		  icon: IconRefresh,
		},
	  ]
	const fontTypes = [
	...
	return (
		<div>
						...
						<Popover.Panel className="absolute left-1/2 z-10 mt-3 w-[30rem] -translate-x-[40%] transform px-4 sm:px-0 lg:max-w-3xl bg-white">
							...
							<div className="relative grid gap-8  bg-white p-7 grid-cols-2">
								...
								<a
									...
									className={`${fontfamily == item.fontTypeChosen ? 'bg-secondary-soft' : 'bg-white'} -m-3 hover:bg-secondary-soft active:bg-secondary flex items-center rounded-lg p-2 transition duration-150 ease-in-out hover:bg-gray-50 focus:outline-none focus-visible:ring focus-visible:ring-orange-500 focus-visible:ring-opacity-50`}
								>
			...
			<div style={{ fontSize: `${fontsize}px`, fontFamily: `${fontfamily}`, backgroundColsor: `${bgColor}`, color: `${txtColor}`, borderColor: `${borderColor}` }} className={`rounded-lg leading-loose p-8 bordser-2 transition-colors text-justify duration-400`}>
